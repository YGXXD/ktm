...
keg::VulkanContext::VulkanContext(void* window)
{
	...
	renderTargetImages.resize(imageCount);
	vkGetSwapchainImagesKHR(device, swapChain, &imageCount, renderTargetImages.data());

	renderTargetViews.resize(imageCount);
	...
	for(int i = 0; i < renderTargetViews.size(); i++)
	{
		...
		vkCreateImageView(device, &ImageViewInfo, nullptr, &renderTargetViews[i]);
...
{
	VkCommandPool graphicsCmdPool = queueCmdPoolMap[graphicsQueue];
