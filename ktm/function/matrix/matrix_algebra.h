...
template <class M>
KTM_CORE_FUNC std::enable_if_t<is_matrix_v<M>, mat_traits_tp_t<M>> transpose(const M& m) noexcept
{
...
template <class M>
KTM_CORE_FUNC std::enable_if_t<is_square_matrix_v<M>, mat_traits_col_t<M>> diagonal(const M& m)
{
    return detail::matrix_algebra_implement::diagonal<mat_traits_col_v<M>, mat_traits_base_t<M>>::call(m);
}

template <class M>
KTM_CORE_FUNC std::enable_if_t<is_square_matrix_v<M>, mat_traits_base_t<M>> trace(const M& m)
{
    return reduce_add(detail::matrix_algebra_implement::diagonal<mat_traits_col_v<M>, mat_traits_base_t<M>>::call(m));
}

template <class M>
KTM_CORE_FUNC std::enable_if_t<is_square_matrix_v<M>, mat_traits_base_t<M>> determinant(const M& m)
{
    return detail::matrix_algebra_implement::determinant<mat_traits_col_v<M>, mat_traits_base_t<M>>::call(m);
}

template <class M>
KTM_CORE_FUNC std::enable_if_t<is_square_matrix_v<M> && is_floating_point_base_v<M>, M> inverse(const M& m)
{
