...
{
    return C(conjugate(c).vector / length_squared(c.vector));
}
...
KTM_INLINE std::enable_if_t<is_complex_v<C>, C> normalize(const C& c) noexcept
{
    ...
    T ls = length_squared(c.vector);
    return ls == zero<T> ? C(zero<T>, one<T>) : C(c.vector * rsqrt(ls));
}
...
template<class C>
KTM_NOINLINE std::enable_if_t<is_complex_v<C>, C> slerp_internal(const C& x, const C& y, comp_traits_base_t<C> t) noexcept
{
    using T = comp_traits_base_t<C>;
    T a = C::from_to(y.vector, x.vector).angle();
    T normal = a < zero<T> ? a + static_cast<T>(2) * pi<T> : a;
...
template<class C>
KTM_NOINLINE std::enable_if_t<is_complex_v<C>, C> slerp_longest(const C& x, const C& y, comp_traits_base_t<C> t) noexcept
{
    using T = comp_traits_base_t<C>;
    T a = C::from_to(y.vector, x.vector).angle();
    T normal = a < zero<T> ? a + static_cast<T>(2) * pi<T> : a - static_cast<T>(2) * pi<T>;
    return C::from_angle(t * normal) * x;
}
