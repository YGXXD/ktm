...
#define KTM_MATRIX_COMPONENT_ELEMENT(name, index)          \
    ...
    inline name##_type& get_##name() noexcept              \
    {                                                      \
        return std::get<index>(*this);                     \
    }                                                      \
    inline const name##_type& get_##name() const noexcept  \
    {                                                      \
        return std::get<index>(*this);                     \
    }
...
template <size_t Row, size_t Col, typename T>
    struct svd_component<mat<Row, Col, T>> : std::tuple < mat<Col, Col, T>,
    vec<Row<Col ? Row : Col, T>, mat<Row, Row, T>>
{
