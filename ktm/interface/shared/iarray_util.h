...
struct iarray_util : Father
{
    ...
    KTM_FUNC constexpr size_t size() const noexcept { return std::tuple_size_v<std::extract_type_t<decltype(to_array())>>; }
    KTM_FUNC constexpr size_t max_size() const noexcept { return std::tuple_size_v<std::extract_type_t<decltype(to_array())>>; }
