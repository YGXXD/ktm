...
struct iquat_data<Father, quat<T>> : Father
{
    ...
    KTM_FUNC T real() const noexcept
    {
        return r;
    }

    KTM_FUNC vec<3, T> imag() const noexcept
    {
        return vec<3, T>(i, j, k);
    }

    KTM_FUNC T angle() const noexcept
    {
        return static_cast<T>(2) * atan2(length(imag()), real());
    }

    KTM_FUNC vec<3, T> axis() const noexcept
    {
        return normalize(imag());
    }

    KTM_FUNC vec<4, T>& operator*() noexcept
    {
        return reinterpret_cast<vec<4, T>&>(st);
    }

    KTM_FUNC const vec<4, T>& operator*() const noexcept
    {
        return reinterpret_cast<const vec<4, T>&>(st);
    }
