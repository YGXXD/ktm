...
struct iquat_data<Father, quat<T>> : Father
{
    ...
    KTM_CORE_FUNC vec<3, T> imag() const noexcept { return vec<3, T>(i, j, k); }

    KTM_CORE_FUNC T angle() const noexcept { return static_cast<T>(2) * atan2(length(imag()), real()); }

    KTM_CORE_FUNC vec<3, T> axis() const noexcept { return normalize(imag()); }

    KTM_CORE_FUNC vec<4, T>& operator*() noexcept { return reinterpret_cast<vec<4, T>&>(st); }

    KTM_CORE_FUNC const vec<4, T>& operator*() const noexcept { return reinterpret_cast<const vec<4, T>&>(st); }
    ...
    KTM_CORE_FUNC mat<4, 4, T> matrix4x4() const noexcept
    {
        mat<4, 4, T> ret {};
