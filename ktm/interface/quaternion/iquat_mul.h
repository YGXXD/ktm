...
struct iquat_mul<Father, quat<T>> : Father
{
    ...
    friend KTM_CORE_FUNC quat<T> operator*(const quat<T>& x, const quat<T>& y) noexcept
    {
        quat<T> ret;
    ...
    friend KTM_CORE_FUNC quat<T>& operator*=(quat<T>& x, const quat<T>& y) noexcept
    {
        detail::quat_mul_implement::mul<T>(x, x, y);
        return x;
    }

    friend KTM_CORE_FUNC vec<3, T> operator*(const quat<T>& q, const vec<3, T>& v) noexcept
    {
        vec<3, T> ret;
