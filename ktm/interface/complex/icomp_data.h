...
struct icomp_data<Father, comp<T>> : Father
{
    ...
    KTM_FUNC T imag() const noexcept
    {
        return i;
    }
    ...
    KTM_FUNC vec<2, T>& operator*() noexcept
    {
        return reinterpret_cast<vec<2, T>&>(st);
    }

    KTM_FUNC const vec<2, T>& operator*() const noexcept
    {
        return reinterpret_cast<const vec<2, T>&>(st);
    }
