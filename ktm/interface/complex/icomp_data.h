...
struct icomp_data<Father, comp<T>> : Father
{
    ...
    KTM_CORE_FUNC constexpr icomp_data() noexcept : i(zero<T>), r(zero<T>) {};
    ...
    KTM_CORE_FUNC constexpr icomp_data(const vec<2, T> vec) noexcept : i(vec.x), r(vec.y) {}

    KTM_CORE_FUNC T real() const noexcept { return r; }

    KTM_CORE_FUNC T imag() const noexcept { return i; }

    KTM_CORE_FUNC T angle() const noexcept { return atan2(imag(), real()); }

    KTM_CORE_FUNC vec<2, T>& operator*() noexcept { return reinterpret_cast<vec<2, T>&>(st); }

    KTM_CORE_FUNC const vec<2, T>& operator*() const noexcept { return reinterpret_cast<const vec<2, T>&>(st); }

    KTM_CORE_FUNC mat<2, 2, T> matrix2x2() const noexcept { return mat<2, 2, T>(vec<2, T>(r, i), vec<2, T>(-i, r)); }

    KTM_CORE_FUNC mat<3, 3, T> matrix3x3() const noexcept
    {
