...
struct imat_make<Father, mat<Row, Col, T, std::enable_if_t<Row != Col>>> : Father
{
    ...
    static KTM_CORE_FUNC
        std::enable_if_t<sizeof...(RowVs) == Col && is_same_vs<vec<Row, T>, std::decay_t<RowVs>...>, mat<Row, Col, T>>
...
private:
    ...
    static KTM_CORE_FUNC void from_row(mat<Row, Col, T>& ret, std::index_sequence<Ns...>, RowVs&&... rows) noexcept
    {
        size_t row_index;
...
struct imat_make<Father, mat<N, N, T>> : Father
{
    ...
    template <typename... RowVs>
    static KTM_CORE_FUNC
        std::enable_if_t<sizeof...(RowVs) == N && is_same_vs<vec<N, T>, std::decay_t<RowVs>...>, mat<N, N, T>>
        from_row(RowVs&&... rows) noexcept
    {
        mat<N, N, T> ret;
        if constexpr (N <= 4)
    ...
    static KTM_CORE_FUNC mat<N, N, T> from_diag(const vec<N, T>& diag) noexcept
    {
        mat<N, N, T> ret {};
        if constexpr (N <= 4)
    ...
    static KTM_CORE_FUNC mat<N, N, T> from_eye() noexcept
    {
        mat<N, N, T> eye {};
        if constexpr (N <= 4)
...
private:
    template <typename... RowVs, size_t... Ns>
    static KTM_CORE_FUNC void from_row(mat<N, N, T>& ret, std::index_sequence<Ns...>, RowVs&&... rows) noexcept
    {
        size_t row_index;
    ...
    template <size_t... Ns>
    static KTM_CORE_FUNC void from_diag(mat<N, N, T>& ret, const vec<N, T>& diag, std::index_sequence<Ns...>) noexcept
    {
        ((ret[Ns][Ns] = diag[Ns]), ...);
    }

    template <size_t... Ns>
    static KTM_CORE_FUNC void from_eye(mat<N, N, T>& ret, std::index_sequence<Ns...>) noexcept
    {
        ((ret[Ns][Ns] = one<T>), ...);
    }
};
