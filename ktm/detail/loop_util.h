...
template<typename R, typename A1, typename T, typename OP>
KTM_FUNC void loop_scalar(R&& self, A1&& l0, const T& scalar, OP&& op, size_t loop)
{
    for(int i = 0; i < loop; ++i)
...
{
    if constexpr(LoopN <= 4)
        loop_scalar(self, std::forward<A1>(l0), scalar, std::forward<OP>(op), std::make_index_sequence<LoopN>());
    else
        loop_scalar(self, std::forward<A1>(l0), scalar, std::forward<OP>(op), LoopN);
}
...
template<typename R, typename A1, typename A2, typename T, typename OP>
KTM_FUNC void loop_scalar(R&& self, A1&& l0, A2&& l1, const T& scalar, OP&& op, size_t loop)
{
    for(int i = 0; i < loop; ++i)
...
{
    if constexpr(LoopN <= 4)
        loop_scalar(self, std::forward<A1>(l0), std::forward<A2>(l1), scalar, std::forward<OP>(op), std::make_index_sequence<LoopN>());
    else
        loop_scalar(self, std::forward<A1>(l0), std::forward<A2>(l1), scalar, std::forward<OP>(op), LoopN);
}
