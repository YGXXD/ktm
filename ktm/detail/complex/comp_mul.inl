...
KTM_INLINE std::enable_if_t<std::is_floating_point_v<T>> 
    ktm::detail::comp_mul_implement::mul(comp<T>& out, const comp<T>& x, const comp<T>& y) noexcept
{
...
template<typename T>
KTM_INLINE std::enable_if_t<std::is_floating_point_v<T>> 
    ktm::detail::comp_mul_implement::act(vec<2, T>& out, const comp<T>& c, const vec<2, T>& v) noexcept
{
