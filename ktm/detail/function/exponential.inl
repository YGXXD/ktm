...
{
    static KTM_CORE_FUNC T call(T x) noexcept { return std::sqrt(x); }
};

template <typename T, typename Void>
struct ktm::detail::exponential_implement::rsqrt
{
    static KTM_CORE_FUNC T call(T x) noexcept { return one<T> / std::sqrt(x); }
};

template <typename T, typename Void>
struct ktm::detail::exponential_implement::recip
{
    static KTM_CORE_FUNC T call(T x) noexcept { return one<T> / x; }
};
...
{
    static KTM_CORE_FUNC T call(T x) noexcept
    {
